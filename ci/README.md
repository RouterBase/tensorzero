...
- From `dummy.yml`, via a dummy job that runs in PR CI only, and always succeeds when it _does_ run.
...
'merge-checks-buildkite'. We set our required status checks to these two jobs _only_, as these jobs propagate failures from their dependencies. This allows us to control which jobs are required from within an individual PR, without touching the repository-level required status checks.
