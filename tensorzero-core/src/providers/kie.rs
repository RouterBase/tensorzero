...
// ── KIE Task Status Query ─────────────────────────────────────────────────────

/// Normalized task status returned to callers of `query_task`.
#[derive(Debug, Clone, Serialize)]
pub struct KieTaskStatus {
...
/// Raw response from GET https://api.kie.ai/api/v1/task/{task_id}
/// Used for video tasks (Kling, Wan, Seedance).
#[derive(Debug, Deserialize)]
struct KIEQueryTaskResponse {
    code: u16,
    #[serde(default)]
    data: Option<KIEQueryTaskData>,
    #[serde(default)]
    message: Option<String>,
}
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct KIEQueryTaskData {
    #[serde(default)]
    status: Option<String>,
    #[serde(default)]
    info: Option<KIEQueryTaskInfo>,
    #[serde(default)]
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct KIEQueryTaskInfo {
    /// May be a JSON-encoded string array `"[\"url\"]"` or a real JSON array.
    result_urls: Option<serde_json::Value>,
}

/// Raw response from GET https://api.kie.ai/api/v1/jobs/recordInfo?taskId={id}
/// Used for image tasks (Flux, Imagen, SeedDream, Ideogram).
#[derive(Debug, Deserialize)]
struct KIERecordInfoResponse {
    code: u16,
    #[serde(default)]
    data: Option<KIERecordInfoData>,
    #[serde(default)]
    msg: Option<String>,
}
...
struct KIERecordInfoData {
    ...
    #[serde(default)]
    state: Option<String>,
    /// JSON-encoded string containing resultUrls array: `{"resultUrls":["url"]}`
    #[serde(default)]
...
impl KieTaskStatus {
    fn from_response(resp: KIEQueryTaskResponse) -> Self {
        // Determine status: use data.status when present, fall back to HTTP code semantics.
        let raw_status = resp
            .data
            .as_ref()
        ...
        let status = if raw_status == "succeed" || resp.code == 200 && !raw_status.is_empty() && raw_status != "processing" && raw_status != "failed" {
            // KIE sometimes omits status on success and just returns code 200 with info
            if raw_status == "succeed" || (resp.code == 200 && raw_status.is_empty()) {
                "succeed"
            } else if raw_status == "failed" || resp.code == 501 || resp.code == 505 {
                "failed"
            } else {
                "processing"
            }
        ...
        let result_urls = resp
            .data
            .as_ref()
            .and_then(|d| d.info.as_ref())
            .and_then(|i| i.result_urls.as_ref())
            .map(parse_kie_url_field)
            .unwrap_or_default();

        let error_message = if status == "failed" {
            resp.data
                .as_ref()
                .and_then(|d| d.fail_reason.clone())
    ...
    fn from_record_info(resp: KIERecordInfoResponse) -> Self {
        let state = resp
            .data
            .as_ref()
        ...
        let status = match state {
            "success" => "succeed",
            "fail" => "failed",
            _ => "processing", // waiting | queuing | generating
        };
        ...
        let result_urls = resp
            .data
            .as_ref()
            .and_then(|d| d.result_json.as_deref())
            .and_then(|s| serde_json::from_str::<serde_json::Value>(s).ok())
            .and_then(|v| v["resultUrls"].as_array().cloned())
        ...
        let error_message = if status == "failed" {
            resp.data
                .as_ref()
                .and_then(|d| d.fail_msg.clone())
                .or_else(|| resp.msg.clone())
        } else {
            None
        };
...
/// Parse KIE's `resultUrls` field which can be:
/// - A JSON string encoding an array: `"[\"url1\",\"url2\"]"`
/// - A real JSON array: `["url1", "url2"]`
/// - A plain string: `"url1"`
fn parse_kie_url_field(val: &serde_json::Value) -> Vec<String> {
    match val {
...
impl KIEProvider {
    /// Query the status of a previously submitted KIE media task.
    ///
    /// KIE's universal task status endpoint works for all model types
    /// (image and video): `GET /api/v1/jobs/recordInfo?taskId={id}`
    ///
    /// Returns normalized status: `"processing"` | `"succeed"` | `"failed"`.
    pub async fn query_task(
    ...
    ) -> Result<KieTaskStatus, crate::error::Error> {
        let api_key = self
            .credentials
            .get_api_key(dynamic_api_keys)
            .map_err(|e| e.log())?;

        let url = format!("{}/api/v1/jobs/recordInfo?taskId={}", *KIE_API_BASE, task_id)
            .parse::<url::Url>()
            .map_err(|e| crate::error::Error::new(crate::error::ErrorDetails::InvalidBaseUrl {
                message: format!("Failed to construct KIE recordInfo URL: {e}"),
            }))?;
        ...
        let response = http_client
            ...
            .send()
            .await
            .map_err(|e| crate::error::Error::new(crate::error::ErrorDetails::InferenceClient {
                message: format!("KIE recordInfo request failed: {e}"),
        ...
        if !status.is_success() {
            return Err(crate::error::Error::new(
                crate::error::ErrorDetails::InferenceServer {
                    message: format!("KIE recordInfo returned {status}: {raw}"),
                    provider_type: PROVIDER_TYPE.to_string(),
                    raw_request: None,
        ...
        let parsed: KIERecordInfoResponse = serde_json::from_str(&raw).map_err(|e| {
            crate::error::Error::new(crate::error::ErrorDetails::InferenceServer {
                message: format!("Failed to parse KIE recordInfo response: {e}"),
                provider_type: PROVIDER_TYPE.to_string(),
                raw_request: None,
        ...
        Ok(KieTaskStatus::from_record_info(parsed))
    }
